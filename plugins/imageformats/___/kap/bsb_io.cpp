...
int fgetc(QDataStream* strm)
{
	char c;
	if(strm->atEnd())
		return -1;
	c = -1;
	strm->readRawData(&c, 1);
	return c;
}
int fseek(QDataStream* strm, long offset, int origin)
{
	bool success;
	QIODevice* dev = strm->device();
	if(origin == SEEK_SET)
		success = dev->seek(offset);
	else if(origin == SEEK_CUR)
		success = dev->seek(int(dev->pos())+offset);
	else if(origin == SEEK_END)
		success = dev->seek(int(dev->size())+offset);
	else
		success = dev->seek(offset);
	if(success)
		return 0;
...
int fread(void* dst, size_t size, size_t num, QDataStream* strm)
{
#if 1
	return strm->readRawData((char*)dst, size*num);
#else
	int n = strm->readRawData((char*)dst, size*num);
	if(n)
		return true;
	return false;
#endif
}
int fputc(int c, QIODevice* dev)
{
	return 0;
}
void fflush(QIODevice* dev)
{
}
...
extern int bsb_get_header_size(QDataStream *fp)
{
	int		text_size = 0, c;
...
/* return 0 on failure */
extern int bsb_open_header(QDataStream* strm, BSBImage *p)
{
